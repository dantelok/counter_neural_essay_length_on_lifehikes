...
class Coh_Model_ILCR_KLD(models.model_base.BaseModel):
    def __init__(self, config, corpus_target, embReader):
        ...
        # self.avg_ent = config.avg_ent
        self.p_map = config.p_map
        self.avg_kld = config.avg_kld
        self.map_kld_essays = config.map_kld_essays
        ...
        #
        fc_in_size = self.encoder_coh.encoder_out_size + 3
        ...
        if corpus_target.output_bias is not None:  # bias
            print(corpus_target.output_bias)
            init_mean_val = np.expand_dims(corpus_target.output_bias, axis=0)
    ...
    def forward(self, text_inputs, mask_input, len_seq, len_sents, tid, mode=""):
        ...
        # get KL-divergence from the pre-calcuated map
        list_klds = []
        for cur_essay_id in tid:
            cur_kld = self.map_kld_essays[int(cur_essay_id)]
            list_klds.append(cur_kld)
        
        kld = torch.FloatTensor(list_klds)
        kld = utils.cast_type(kld, FLOAT, self.use_gpu)
        
        #### Fully Connected
        ilc_vec = torch.cat((ilc_vec, kld), dim=1)

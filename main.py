...
import corpus.corpus_language_strength
...
from corpus.dataset_language_strength import Dataset_LS
...
def get_corpus_target(config):
    ...
    elif config.corpus_target.lower() == "language_strength":
        logger.info("Corpus: Language Strength")
        corpus_target = corpus.corpus_language_strength.CorpusLS(config)
...
def get_dataset(config, id_corpus, pad_id):
    ...
    if config.corpus_target.lower() == "asap":
        ...
        dataset_train = Dataset_LS(id_corpus["train"], config, pad_id)
        dataset_valid = Dataset_LS(id_corpus["valid"], config, pad_id)
        dataset_test = Dataset_LS(id_corpus["test"], config, pad_id)

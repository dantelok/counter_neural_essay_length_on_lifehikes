...
class CorpusLS(CorpusBase):
    """ Corpus class for TOEFL dataset """
    """ prompt: 1 to 8 (p1 to p8 in the paper) """

    #
    def __init__(self, config):
        super(CorpusLS, self).__init__(config)

        self.score_ranges = {
            1: (0, 10), 2: (0, 10), 3: (0, 10), 4: (0, 10), 5: (0, 10), 6: (0, 10), 7: (0, 10), 8: (0, 10)  # low, medium, high
        }
    ...
    def _read_dataset(self, config):
        ...
        # cur_path_ls = os.path.join(path_data, "fold_" + str(config.cur_fold))

        # read
        # str_cur_fold = str(config.cur_fold)
        self.train_total_pd = pd.read_csv(os.path.join(path_data, "language-clean.csv"), sep=",", header=0,
                                          encoding="utf-8", engine='c')  # skip the first line
        self.valid_total_pd = pd.read_csv(os.path.join(path_data, "language_strength.csv"), sep=",", header=0, encoding="utf-8",
                                          engine='c')
        self.test_total_pd = pd.read_csv(os.path.join(path_data, "Dante.csv"), sep=",", header=0, encoding="utf-8",
                                         engine='c')
        ...
        # convert to id
        self.train_corpus = self._sent_split_corpus(self.train_pd['transcripts'].values)  # (doc_id, list of sents)
        self.valid_corpus = self._sent_split_corpus(self.valid_pd['transcripts'].values)
        self.test_corpus = self._sent_split_corpus(self.test_pd['transcripts'].values)
